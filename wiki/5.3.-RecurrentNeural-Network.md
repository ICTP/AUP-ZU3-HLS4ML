...
Open the notebook `rnn-training.ipynb` located in the
**03-rnn/01.training** folder.
...
2. Open `rnn-training.ipynb` located in the **03-rnn/01.training** folder.
...
└── 05.pynq
    ├── 03-rnn.ipynb
    ├── bd_wrapper_embedding.xsa
    ├── recomender_test_sequences.csv
    ├── recomender_test_targets.csv
    └── utils.py
...
In this project, the compressed model is produced through a combination of pruning and post-quantization. Once the compressed model is obtained, the next step is to generate the corresponding High-Level Synthesis (HLS) project, which will produce the IP core responsible for executing inference on the FPGA.
...
**Files:**
1. `03-rnn/01.training/rnn-training.ipynb` (Model training and compression)
2. `03-rnn/02.hls4ml/rnn-hls4ml.ipynb` (hls4ml conversion and IP core generation)
...
17.  Add the IP core to the block design. For this project, search for **`myproject_embedding_accel`**. 
...
<p align="center">
  <img src="img/General/conn_1.png" width="40%">
</p>


<p align="center">
  <img src="img/General/conn_2.png" width="40%">
</p>

The final connections between DMA and the Inference IP core should look as follows:

<p align="center">
  <img src="img/General/conn_3.png" width="40%">
</p>
...
3. Browse to `03-rnn/02.hls4ml/output/myproject_accel_prj/solution1/impl` directory, select it, and finally click on the **Select** button. This directory contains the IP core exported from the HLS project. 

> **Note:**  
> A pre-generated IP core is provided in the **`03-rnn/04.hw/ip`** folder (unzip it before adding to Vivado).
...
4. The **RNN_inference IP** core should appear under **IPs** in the **Add Repository** window.
...
``` bash
cd <user-directory>/aup-zu3-amd-ml/03-rnn/04.hw
```

4. A TCL file can be sourced from Vivado. To recreate the block design you will need a `tcl` command file, that in this case is **rnn_bd.tcl**. To recreate the block design, type in the Tcl console:
...
10. In the following window, change the _XSA file name_ to **bd_wrapper_embedding.xsa**. Click **Next**, and then **Finish**.
...
For the recommender project, upload the contents of 03-rnn/05.pynq into the Jupyter home area. It is recommended to create a dedicated folder to keep the files organized.

> **Note:**  
> The `.xsa` file may be either the one generated by the user or the pre-provided file included in the repository within the **`03-rnn/04.hw`** folder.
...
Inside the PYNQ framework, create a folder named `test_dataset`, move the `.csv` files into it, open the `03-rnn.ipynb` notebook and continue from there.
