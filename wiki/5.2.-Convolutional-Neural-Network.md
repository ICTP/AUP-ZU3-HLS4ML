...
Open the notebook ``cnn-training.ipynb`` located in the
**02-cnn/01.training** folder.
...
2. Open `cnn-training.ipynb` located in the **02-cnn/01.training** folder.
...
│   └── models
│       ├── distilled-mnist.h5
│       └── teacher_model_2Dconv_mnist.h5
├── 02.hls4ml
│   ├── cnn-hls4ml.ipynb
│   └── output
├── 03.hls
│   ├── build_accel.tcl
│   ├── myproject_mnist_accel.cpp
│   └── myproject_mnist_accel.h
├── 04.hw
│   ├── cnn_bd.tcl
│   └── ip
└── 05.pynq
    ├── 02-cnn.ipynb
    ├── bd_wrapper_mnist.xsa
    ├── mnist_dataset.csv
    └── utils.py
...
**Files:**
1. `02-cnn/01.training/cnn-training.ipynb` (Model training and compression)
2. `02-cnn/02.hls4ml/cnn-hls4ml.ipynb` (hls4ml conversion and IP core generation)
...
17.  Add the IP core to the block design. For this project, search for **`myproject_mnist_accel`**. 


<p align="center">
  <img src="img/components/cnn_ip.png" width="30%">
</p>
...
<p align="center">
  <img src="img/General/conn_1_cnn.png" width="40%">
</p>


<p align="center">
  <img src="img/General/conn_2_cnn.png" width="40%">
</p>

The final connections between DMA and the Inference IP core should look as follows:

<p align="center">
  <img src="img/General/conn_3_cnn.png" width="40%">
</p>


Finally, the complete block design is illustrated in the following figure. 

![alt text](img/components/cnn_bd.png)
...
3. Browse to `02-cnn/02.hls4ml/output/myproject_accel_prj/solution1/impl` directory, select it, and finally click on the **Select** button. This directory contains the IP core exported from the HLS project.

> **Note:**  
> A pre-generated IP core is provided in the **`02-cnn/04.hw/ip`** folder (unzip it before adding to Vivado).

<p align="center">
  <img src="img/components/cnn_ip.png" width="30%">
</p>

4. The **MNIST_inference IP** core should appear under **IPs** in the **Add Repository** window.
...
``` bash
cd <user-directory>/aup-zu3-amd-ml/02-cnn/04.hw
```

4. A TCL file can be sourced from Vivado. To recreate the block design you will need a `tcl` command file, that in this case is **cnn_bd.tcl**. To recreate the block design, type in the Tcl console:
...
10. In the following window, change the _XSA file name_ to **bd_wrapper_mnist.xsa**. Click **Next**, and then **Finish**.
...
For the MNIST project, upload the contents of 02-cnn/05.pynq into the Jupyter home area. It is recommended to create a dedicated folder to keep the files organized.


> **Note:**  
> The `.xsa` file may be either the one generated by the user or the pre-provided file included in the repository within the **`02-cnn/04.hw`** folder.
...
Inside the PYNQ framework, create a folder named `test_dataset`, move `mnist_dataset.csv` into it, open the `02-cnn.ipynb` notebook and continue from there.
