...
The main steps involved in the training process are illustrated in the following image, along with their corresponding functions.

![alt text](img/General/model_training_steps.png)
...
- **loss**: A metric that measures how far the model’s predictions are from the actual values. The following loss functions are commonly used for classification tasks:

    - binary_crossentropy: used for binary classification problems (two classes), where labels are typically 0 or 1.

    - sparse_categorical_crossentropy: used for multi-class classification when labels are provided as integers.

    - categorical_crossentropy: used for multi-class classification when labels are one-hot encoded.
...
![alt text](img/General/hls4ml_integration.png)

**hls4ml** is an open-source framework that converts trained machine learning models (especially neural networks) into High Level Synthesis (HLS) code so they can run efficiently on FPGAs. It takes models trained in frameworks such as TensorFlow/Keras or PyTorch and automatically generates optimized C/C++ code that can be synthesized with FPGA toolchains (e.g., Xilinx Vitis/Vivado HLS or Intel HLS).

The main goal of hls4ml is to enable low-latency and resource-efficient inference on FPGA or SoC platforms. It is widely used in applications that require fast, real-time processing, such as high-energy physics triggers and edge AI systems.

After synthesizing the hls4ml project and reviewing the hardware metrics (latency, throughput, and LUT/FF/DSP/BRAM utilization), the next step is to proceed with FPGA integration. 
...
The *build_accel.tcl* script defines the complete HLS build process: it creates a clean Vitis HLS project, adds the **hls4ml** sources plus the AXI/DMA wrapper, configures the FPGA part, sets the clock, and triggers synthesis. Also,  it generates a fully packaged IP core that can be directly imported into Vivado.


### Template Vivado Project
